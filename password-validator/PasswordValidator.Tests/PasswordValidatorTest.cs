...
namespace PasswordValidator.Tests
{
    public class PasswordValidatorTest
    {
        ...
        {
            Assert.That(PasswordValidatorFactory.WithAllRules().IsValid("1Aa_5678"), Is.False);
        }
        ...
        {
            Assert.That(PasswordValidatorFactory.WithAllRules().IsValid(password), Is.True);
        }

        [Test]
        public void WithoutUnderscore_IsNotValid()
        {
            Assert.That(PasswordValidatorFactory.WithAllRules().IsValid("1Aaa56789"), Is.False);
        }
        
        [Test]
        public void WithoutNumbers_IsNotValid()
        {
            Assert.That(PasswordValidatorFactory.WithAllRules().IsValid("aAa_aaaaa"), Is.False);
        }

        [Test]
        public void WithoutLowercase_IsNotValid()
        {
            Assert.That(PasswordValidatorFactory.WithAllRules().IsValid("1A__56789"), Is.False);
        }

        [Test]
        public void WithoutUppercase_IsNotValid()
        {
            Assert.That(PasswordValidatorFactory.WithAllRules().IsValid("1aa_56789"), Is.False);
        }
    }
}